...
WITH 
	vars AS (SELECT '(^.*\S)(?:.| )([IΙ]{1})$'::text AS regex),
	r AS (select word.*, regexp_matches(line, vars.regex) as m 
...
-----------
select * from word where variant ~ '[⁰¹²³⁴⁵⁶⁷⁸⁹]';
select * from word where version ~ '[ⅠⅡⅢⅣ]';
select * from word where not variant ~ '[ⅠⅡⅢⅣ]' and version is not null;
--update word set variant=null where version ~ '[ⅠⅡⅢⅣ]';
...
from word where line like '===' and alt_rest like '%черед%' and deprecated is null
and alt_rest ~ '(?:черед.|; |, |,|)(?:|)([а-ё’]+((|)|)(–|-)(|)[а-ё]+)'
--and not alt_rest ~ '^((?:черед.|; |, |,|)(?:|)([а-ё’]+)(?:(|)|)(?:–|-)(?:|)([а-ё]+))+$'
;

;WITH 
	vars AS (SELECT '(?:черед.|)(?:|)([а-ё]+)(?:(?:|)|)(?:–|-)(?:|)([а-ё]+)(?:[^а-ё]+)([а-ё]+)(?:(?:|)|)(?:–|-)(?:|)([а-ё]+)'::text AS regex)
--parts as (
...
from word, vars where alt_rest like '%черед%' and deprecated is null
and alt_rest ~ '.+[а-ё]+.*(–|-).*[а-ё]+.*[а-ё]+.*(–|-).*[а-ё]+.*'
