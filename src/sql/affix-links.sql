with
  aa as (
	select count(*) qty, word, array_agg(id) aa, array_agg(affix) a,
		array(select unnest(array_agg(offs)) v order by v) starts,
		array(select unnest(array_agg(len)) v order by v) lens,
		array(select unnest(array_agg(offs+len)) v order by v) ends
